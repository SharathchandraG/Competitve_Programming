...
class TempTracker(object):
    ...
    def insert(self, temperature):
        ...
        self.mean = (self.count * self.mean + temperature)/(self.count + 1) 
        self.count += 1         
        if not self.dict_1:
            self.mode = temperature
        if temperature in self.dict_1:
            self.dict_1[temperature] += 1   
        else:
            self.dict_1[temperature] = 1
            
        if self.dict_1[temperature] > self.dict_1[self.mode]:
            self.mode = temperature   
